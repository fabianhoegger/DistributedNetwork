...
{
    this->sending_id = other.sending_id;
    this->sending_neighbours = other.sending_neighbours;
    this->pos = other.pos;
    for (unsigned int i=0; i<10; i++)
        this->routing[i] = other.routing[i];
}

void MyPacket::parsimPack(omnetpp::cCommBuffer *b) const
{
    ::omnetpp::cPacket::parsimPack(b);
    doParsimPacking(b,this->sending_id);
    doParsimPacking(b,this->sending_neighbours);
    doParsimPacking(b,this->pos);
    doParsimArrayPacking(b,this->routing,10);
}

void MyPacket::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ::omnetpp::cPacket::parsimUnpack(b);
    doParsimUnpacking(b,this->sending_id);
    doParsimUnpacking(b,this->sending_neighbours);
    doParsimUnpacking(b,this->pos);
    doParsimArrayUnpacking(b,this->routing,10);
}

int MyPacket::getSending_id() const
{
    return this->sending_id;
}

void MyPacket::setSending_id(int sending_id)
{
    this->sending_id = sending_id;
}

int MyPacket::getSending_neighbours() const
{
    return this->sending_neighbours;
}

void MyPacket::setSending_neighbours(int sending_neighbours)
{
    this->sending_neighbours = sending_neighbours;
}

int MyPacket::getPos() const
{
    return this->pos;
}

void MyPacket::setPos(int pos)
{
    this->pos = pos;
}

unsigned int MyPacket::getRoutingArraySize() const
{
    return 10;
}

int MyPacket::getRouting(unsigned int k) const
{
    if (k>=10) throw omnetpp::cRuntimeError("Array of size 10 indexed by %lu", (unsigned long)k);
    return this->routing[k];
}

void MyPacket::setRouting(unsigned int k, int routing)
{
    if (k>=10) throw omnetpp::cRuntimeError("Array of size 10 indexed by %lu", (unsigned long)k);
    this->routing[k] = routing;
}
...
int MyPacketDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0]=='s' && strcmp(fieldName, "sending_id")==0) return base+0;
    if (fieldName[0]=='s' && strcmp(fieldName, "sending_neighbours")==0) return base+1;
    if (fieldName[0]=='p' && strcmp(fieldName, "pos")==0) return base+2;
    if (fieldName[0]=='r' && strcmp(fieldName, "routing")==0) return base+3;
...
std::string MyPacketDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    MyPacket *pp = (MyPacket *)object; (void)pp;
    switch (field) {
        case 0: return long2string(pp->getSending_id());
        case 1: return long2string(pp->getSending_neighbours());
        case 2: return long2string(pp->getPos());
        case 3: return long2string(pp->getRouting(i));
        default: return "";
    }
}
...
bool MyPacketDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    MyPacket *pp = (MyPacket *)object; (void)pp;
    switch (field) {
        case 0: pp->setSending_id(string2long(value)); return true;
        case 1: pp->setSending_neighbours(string2long(value)); return true;
        case 2: pp->setPos(string2long(value)); return true;
        case 3: pp->setRouting(i,string2long(value)); return true;
